...
export default function ComisionesPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Page header */}
      <div>
        <h1 className="text-2xl font-black text-slate-900 tracking-tight">Comisiones</h1>
        <p className="text-sm text-slate-500 mt-1">Gestion de ventas, exclusiones y calculo de comisiones por vendedor</p>
      </div>

      {/* Tabs */}
      <div className="flex flex-wrap items-center gap-2 bg-slate-100 rounded-xl p-1.5 w-fit">
        <TabButton active={activeTab === "ventas"} onClick={() => setActiveTab("ventas")} icon={Receipt}>Ventas</TabButton>
        <TabButton active={activeTab === "recaudo"} onClick={() => setActiveTab("recaudo")} icon={Wallet}>Recaudo</TabButton>
        <TabButton active={activeTab === "exclusiones"} onClick={() => setActiveTab("exclusiones")} icon={Ban}>Exclusiones</TabButton>
        <TabButton active={activeTab === "catalogo"} onClick={() => setActiveTab("catalogo")} icon={BookOpen}>Catalogo</TabButton>
        <TabButton active={activeTab === "reporte"} onClick={() => setActiveTab("reporte")} icon={CalendarRange}>Reporte Mensual</TabButton>
        <TabButton active={activeTab === "presupuestos"} onClick={() => setActiveTab("presupuestos")} icon={Settings}>Presupuestos</TabButton>
      </div>
