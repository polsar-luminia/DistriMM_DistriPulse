...
export function calculateKPIs(items) {
  const total = roundCOP(
    items.reduce((sum, item) => sum + (item.valor_saldo || 0), 0),
  );
  const vencidaItems = items.filter((i) => (i.dias_mora || 0) > 0);
  const vencida = roundCOP(
    vencidaItems.reduce((sum, item) => sum + (item.valor_saldo || 0), 0),
  );
...
export function buildClientMap(items) {
  const clientMap = {};
  items.forEach((item) => {
    // Usar NIT como key estable (evita fusionar clientes homónimos)
    const clientKey = item.tercero_nit || item.cliente_nombre || "Desconocido";
    ...
    clientMap[clientKey].deuda += item.valor_saldo || 0;
    clientMap[clientKey].items.push(item);
    if ((item.dias_mora || 0) > clientMap[clientKey].maxMora) {
      clientMap[clientKey].maxMora = item.dias_mora || 0;
    }
  });
...
export function calculatePareto(
  ...
  return uniqueClientsCount > 0
    ? Math.round((paretoClientsCount / uniqueClientsCount) * 1000) / 10
    : 0;
}
...
export function calculateAging(items) {
  ...
  return Object.keys(buckets).map((key) => ({
    ...
    percent:
      totalAging > 0
        ? parseFloat(((buckets[key].val / totalAging) * 100).toFixed(1))
        : 0,
  }));
}
...
export function calculateProjection(items) {
        ...
        if (typeof item.fecha_vencimiento === "string") {
          ...
          if (parts.length === 3) {
            ...
            if (itemDate >= today && itemDate <= next30Days) {
              ...
              projectionMap[k] =
                (projectionMap[k] || 0) + (item.valor_saldo || 0);
            }
          }
        }
      } catch {
  ...
  return Object.keys(projectionMap)
    .sort()
    .map((date) => ({ date, total: projectionMap[date] }));
}
...
export function buildLists(items, clientMap) {
  const urgentItems = Object.values(clientMap)
    ...
    .map((c, idx) => ({
      ...
      days: c.maxMora,
      reason:
        c.maxMora > THRESHOLDS.LEGAL_ACTION_DAYS
          ? "Cobro Jurídico"
          : "Riesgo Alto",
    }));
...
export function buildRadarData(sortedClients) {
  const radarSubset = sortedClients.filter((c) => c.maxMora > 0).slice(0, 5);
...
export function calculateMoraPonderada(items) {
  ...
  return totalBalance > 0
    ? Math.round((weightedSum / totalBalance) * 100) / 100
    : 0;
}
...
export function buildVendedorStats(items) {
  ...
  const uniqueVendedores = [
    ...new Set(items.map((i) => i.vendedor_codigo).filter(Boolean)),
  ];
