...
export default function CatalogoUploadModal({ isOpen, onClose, onSuccess }) {
  ...
  const reset = () => { setFile(null); setStep("select"); setPreviewData([]); setFullData([]); setError(null); setProgress(0); };
  ...
  const handleAnalyze = () => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (jsonData.length === 0) jsonData = XLSX.utils.sheet_to_json(ws, { range: 0 });
        ...
        if (processed.length === 0) throw new Error("No se encontraron productos validos.");
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      for (let i = 0; i < fullData.length; i += batchSize) {
        ...
        if (err) throw err;
        setProgress(20 + Math.round(((i + batch.length) / fullData.length) * 75));
      }
      setProgress(100);
      setStep("success");
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl my-8 overflow-hidden flex flex-col max-h-[90vh]">
        <div className="bg-slate-900 p-4 flex justify-between items-center text-white shrink-0">
          <h3 className="font-bold text-lg flex items-center gap-2">
            <Package size={20} className="text-emerald-400" /> Cargar Catalogo de Productos
          </h3>
        ...
        <div className="p-6 overflow-y-auto">
          ...
          {step === "select" && (
            <div className="space-y-6">
              <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 text-sm text-emerald-700">
                ...
                <p className="text-xs text-emerald-500">Se extraen: Codigo (col 1), Nombre (col 4), Categoria (col 9-10), Marca (col 17). Productos existentes se actualizan.</p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className={cn("border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center text-center transition-all group", file ? "border-emerald-500 bg-emerald-50/50" : "border-slate-300 hover:border-emerald-400 hover:bg-slate-50")}>
                  <input type="file" accept=".xlsx,.xls,.csv" onChange={handleFileChange} className="hidden" id="catalogo-file" />
              ...
              <button onClick={handleAnalyze} disabled={!file} className="w-full py-3 bg-emerald-600 text-white rounded-lg text-sm font-bold hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg shadow-emerald-900/20">
          ...
          {step === "preview" && (
            <div className="space-y-6">
              <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 flex gap-3">
                ...
                <div>
                  <h4 className="font-bold text-sm text-emerald-800">Catalogo Detectado</h4>
                  <p className="text-sm text-emerald-700">{fullData.length} productos encontrados. Los existentes se actualizaran.</p>
                </div>
              </div>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {previewData.map((r, i) => (
                        <tr key={i} className="hover:bg-slate-50">
                          ...
                          <td className="px-3 py-2 text-xs truncate max-w-[200px]">{r.nombre}</td>
                          <td className="px-3 py-2 text-xs">{r.categoria_nombre}</td>
              ...
              <div className="flex gap-3">
                <button onClick={() => setStep("select")} className="flex-1 px-4 py-3 border border-slate-300 text-slate-700 rounded-lg text-sm font-bold hover:bg-slate-50">Cancelar</button>
                <button onClick={handleUpload} disabled={step === "uploading"} className="flex-[2] px-4 py-3 bg-emerald-600 text-white rounded-lg text-sm font-bold hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-xl shadow-emerald-900/20 flex items-center justify-center gap-2">
          ...
          {step === "uploading" && (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <h4 className="text-xl font-bold text-slate-900 mb-2">Guardando Catalogo...</h4>
          ...
          {step === "success" && (
            <div className="flex flex-col items-center justify-center py-12 text-emerald-600">
              ...
              <p className="text-slate-500">{fullData.length} productos procesados.</p>
            </div>
          )}
        </div>
      </div>
