...
export default function RecaudoUploadModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleAnalyze = () => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Row 0 is decorative title; real headers on row 1, data from row 2
        ...
        if (processed.length === 0) throw new Error("No se encontraron registros validos.");
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (createdId) {
        await supabase.from("distrimm_comisiones_cargas_recaudo").delete().eq("id", createdId);
      }
  ...
  const totalRecaudado = fullData.reduce((s, r) => s + r.valor_recaudo, 0);
  const totalComisionable = fullData.filter((r) => r.aplica_comision).reduce((s, r) => s + r.valor_recaudo, 0);
        ...
        <div className="p-6 overflow-y-auto">
          ...
          {step === "select" && (
            <div className="space-y-6">
              <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 text-sm text-emerald-700">
                <p className="font-bold mb-1">Excel "Comisiones x Cartera" (Recibos de Caja)</p>
                <p className="text-xs text-emerald-600">Se omite la fila decorativa (titulo). Filas con dias_mora &gt; {DIAS_MORA_LIMITE} se marcan como no comisionables.</p>
              </div>

              <div className="space-y-2">
                <label className="block text-sm font-bold text-slate-700">Periodo de Recaudo</label>
                ...
                <div className={cn("border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center text-center transition-all group", file ? "border-emerald-500 bg-emerald-50/50" : "border-slate-300 hover:border-emerald-400 hover:bg-slate-50")}>
                  ...
                  <label htmlFor="recaudo-file" className="cursor-pointer flex flex-col items-center w-full">
                    {file ? (
                      ...
                      <>
                        <div className="bg-slate-100 p-3 rounded-full mb-3 group-hover:bg-emerald-100 transition-colors"><Upload size={32} className="text-slate-400 group-hover:text-emerald-600 transition-colors" /></div>
                        <span className="text-sm font-semibold text-slate-700">Haz clic para buscar el archivo</span>
                        <span className="text-xs text-slate-400 mt-2">Soporta .xls / .xlsx</span>
                      </>
                    )}
                  </label>
                </div>
              ...
              <button onClick={handleAnalyze} disabled={!file} className="w-full py-3 bg-emerald-600 text-white rounded-lg text-sm font-bold hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg shadow-emerald-900/20 transition-all">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 flex gap-3">
                ...
                <div className="min-w-0">
                  <h4 className="font-bold text-sm uppercase tracking-wide mb-1 text-amber-800">Recaudos Detectados</h4>
                  <p className="text-sm text-amber-700">
                    {fullData.length} recibos — {countComisionable} comisionables ({formatFullCurrency(totalComisionable)}),{" "}
                    {countExcluido} excluidos (&gt;{DIAS_MORA_LIMITE}d mora). Total recaudado: {formatFullCurrency(totalRecaudado)}
                  </p>
                </div>
              </div>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {previewData.map((row, i) => (
                        <tr key={i} className="hover:bg-slate-50">
                          <td className="px-3 py-2 text-xs font-medium">{row.vendedor_codigo}</td>
                          <td className="px-3 py-2 text-xs truncate max-w-[140px]">{row.cliente_nombre || row.cliente_nit}</td>
                          <td className="px-3 py-2 text-xs font-mono">{row.factura}</td>
                          <td className="px-3 py-2 text-xs font-mono">{row.fecha_abono}</td>
                          <td className="px-3 py-2 text-xs text-right font-mono">{formatFullCurrency(row.valor_recaudo)}</td>
                          <td className="px-3 py-2 text-xs text-center font-mono">
                            <span className={row.dias_mora > DIAS_MORA_LIMITE ? "text-rose-600 font-bold" : "text-slate-600"}>
                              {row.dias_mora}
                            </span>
                          </td>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={handleUpload} disabled={uploading} className="flex-[2] px-4 py-3 bg-emerald-600 text-white rounded-lg text-sm font-bold hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-xl shadow-emerald-900/20 flex items-center justify-center gap-2 transition-all">
          ...
          {step === "uploading" && (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <h4 className="text-xl font-bold text-slate-900 mb-2">Guardando Recaudos...</h4>
          ...
          {step === "success" && (
            <div className="flex flex-col items-center justify-center py-12 text-emerald-600">
              ...
              <p className="text-slate-500">{fullData.length} recaudos guardados correctamente.</p>
            </div>
          )}
        </div>
      </div>
