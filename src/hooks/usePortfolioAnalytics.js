...
export function usePortfolioAnalytics() {
  ...
  const processPortfolioData = useCallback((items, loadId, allLoads) => {
    // Para cargas históricas, usar fecha_corte como referencia en vez de "hoy"
    const currentLoad = allLoads?.find((l) => l.id === loadId);
    const fechaCorte = currentLoad?.fecha_corte || null;
    const processedItems = preprocessItems(items, fechaCorte);
    const kpis = calculateKPIs(processedItems);
    const { clientMap, sortedClients, uniqueClientsCount } =
      buildClientMap(processedItems);
    const paretoPercentage = calculatePareto(
      sortedClients,
      kpis.total,
      uniqueClientsCount,
    );
    ...
    const { urgentItems, aggregatedClients, upcomingItems } = buildLists(
      processedItems,
      clientMap,
    );
    const radarData = buildRadarData(sortedClients);
    const topOldest = buildTopOldest(processedItems);
    const { vendedorStats, uniqueVendedores } =
      buildVendedorStats(processedItems);
    ...
    setData({
      ...
      charts: {
        ...
        invoicesByStatus: [
          {
            name: "Al Día",
            value: processedItems.length - kpis.vencidaItems.length,
            color: COLORS.CHART.PRIMARY,
          },
          {
            name: "Vencidas",
            value: kpis.vencidaItems.length,
            color: COLORS.CHART.DANGER,
          },
        ],
        radarData,
        topOldest,
  ...
  const fetchData = useCallback(
    async (loadId, allLoads, requestId) => {
      if (!loadId) return;
      ...
      try {
        ...
        if (requestId !== undefined && requestId !== loadRequestIdRef.current)
          return;
  ...
  const changeLoad = useCallback(
    (newLoadId) => {
      ...
      currentLoadIdRef.current = newLoadId;
      fetchData(newLoadId, dataRef.current.availableLoads, requestId);
    },
    [fetchData],
  );
  ...
  const handleDeleteLoad = useCallback(
    async (loadIdToDelete) => {
      ...
      if (success) {
        ...
        const updatedLoads = latestLoads.filter(
          (load) => load.id !== loadIdToDelete,
        );
        ...
        currentLoadIdRef.current = newCurrentId;
