...
DROP FUNCTION IF EXISTS fn_upload_recaudos(JSONB, JSONB);
DROP FUNCTION IF EXISTS fn_upload_recaudos(JSONB, JSONB, TEXT);

CREATE OR REPLACE FUNCTION fn_upload_recaudos(
  p_carga JSONB,
  p_recaudos JSONB,
  p_origen TEXT DEFAULT 'credito'
) RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  v_carga_id UUID;
  v_old_ids UUID[];
  v_origen TEXT := COALESCE(NULLIF(p_origen, ''), 'credito');
BEGIN
  ...
  WHERE EXTRACT(YEAR FROM fecha_periodo) = EXTRACT(YEAR FROM (p_carga->>'fecha_periodo')::DATE)
    ...
    AND COALESCE(origen, 'credito') = v_origen
  FOR UPDATE;
  ...
  INSERT INTO distrimm_comisiones_recaudos (
    ...
    aplica_comision, origen, periodo_year, periodo_month
  )
  SELECT
    v_carga_id,
    r->>'vendedor_codigo', r->>'cliente_nit', r->>'cliente_nombre',
    r->>'factura', r->>'comprobante',
    safe_date(r->>'fecha_abono'),
    safe_date(r->>'fecha_cxc'),
    safe_date(r->>'fecha_vence'),
