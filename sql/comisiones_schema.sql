-- ============================================================================
-- DistriMM Comisiones Module — Schema
-- Applied via Supabase MCP on 2026-02-25
-- ============================================================================
...
-- Catalogo maestro de productos (del Excel "Saldos de Productos")
...
-- Reglas de exclusion para el calculo de comisiones
...
CREATE POLICY "Authenticated full access on distrimm_comisiones_cargas"
  ON distrimm_comisiones_cargas FOR ALL
  USING (auth.role() = 'authenticated')
  WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Authenticated full access on distrimm_comisiones_ventas"
  ON distrimm_comisiones_ventas FOR ALL
  USING (auth.role() = 'authenticated')
  WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Authenticated full access on distrimm_productos_catalogo"
  ON distrimm_productos_catalogo FOR ALL
  USING (auth.role() = 'authenticated')
  WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Authenticated full access on distrimm_comisiones_exclusiones"
  ON distrimm_comisiones_exclusiones FOR ALL
  USING (auth.role() = 'authenticated')
  WITH CHECK (auth.role() = 'authenticated');

-- ============================================================================
-- 4. RPC: Calculo de comisiones por vendedor
-- ============================================================================
